...
train_loader = torch.utils.data.DataLoader(datasets.MNIST('mnist-data/', train=True, download=True, transform=AlexTransform), batch_size=500, shuffle=True)

test_loader = torch.utils.data.DataLoader(datasets.MNIST('mnist-data/', train=False, download=True, transform=AlexTransform), batch_size=1, shuffle=False)
...
softplus = torch.nn.Softplus()#TODO:可看作是ReLU函数的平滑

#TODO: return      pyro.random_module("module", net, priors)
def guide(x_data, y_data):
    # First layer weight distribution priors
    convLayer1w_mu = torch.randn_like(net.conv1[0].weight) #TODO：torch.randn_like()返回一个与输入张量大小相同的张量，其中填充了均值为0方差为1的正态分布的随机值。
    ...
    fc1Layer_w = Normal(loc=fc1w_mu_param, scale=fc1w_sigma_param).independent(1)#TODO：这里难道是全连接层所特有的？
...
model_path = 'Bayesian_models' # This should be the path where the models are located   TODO:模型How来的呢？？？？？？
saved_model_dict = torch.load(model_path + '/model_60.pt')#, map_location=torch.device('cpu'))
net.load_state_dict(saved_model_dict['model'])
guide = saved_model_dict['guide']
pyro.get_param_store().load(model_path + "/model_60_params.pt")
